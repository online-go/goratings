...
class Glicko2Entry:
    ...
    def after_aging_to_timestamp(
        self, timestamp: int | None, minus_one_period: bool = False
    ) -> "Glicko2Entry":
        ...
        if timestamp and AGING_PERIOD_SECONDS and minus_one_period:
            timestamp = max(
                self.timestamp if self.timestamp else 0,
                timestamp - AGING_PERIOD_SECONDS,
            )
        copy = self.copy()
    ...
    def expand_deviation(
        self, age: int, override_aging_period: int | None = None
    ) -> "Glicko2Entry":
    ...
    def expected_win_probability(
        self, white: "Glicko2Entry", handicap_adjustment: float, ignore_g: bool = False
    ) -> float:
...
def _age_phi(
    ...
    volatility: float,
    age: int | None,
    override_aging_period: int | None = None,
) -> float:
    # Implementation as defined by [glicko2], but converted to closed form,
    # allowing deviation to expand continuously over fractional periods.
    #
    ...
    if age is not None:
        assert age >= 0
        aging_period = (
            AGING_PERIOD_SECONDS
            if override_aging_period is None
            else override_aging_period
        )
        if aging_period:
...
def glicko2_update(
    player: Glicko2Entry,
    matches: List[Tuple[Glicko2Entry, int]],
    timestamp: int | None = None,
) -> Glicko2Entry:
...
def glicko2_configure(
    tao: float, min_rd: float, max_rd: float, aging_period_days: float
) -> None:
    global TAO
    global MIN_RD
    global MAX_RD
    ...
    MAX_RD = max_rd
    AGING_PERIOD_SECONDS = (
        int(aging_period_days * 24 * 60 * 60) if aging_period_days else None
    )
